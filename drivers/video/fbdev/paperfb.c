...
static int paperfb_setupfb(struct paperfb_dev *fbdev)
{
	...
	paperfb_writereg(fbdev, AH_HVACT_REG_ADDR, (var->xres << 16) + var->yres);

	/* Total length and width of frame */
	hlen = var->left_margin + var->right_margin + var->hsync_len + var->xres;
	vlen = var->upper_margin + var->lower_margin + var->vsync_len + var->yres;
	paperfb_writereg(fbdev, AH_HVTOT_REG_ADDR, (hlen << 16) + vlen);

	//TODO: Is this right???
	paperfb_writereg(fbdev, AH_HVFRONT_REG_ADDR, (var->right_margin << 16) + var->lower_margin);
